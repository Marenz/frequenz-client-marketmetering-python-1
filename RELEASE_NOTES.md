...
- `stream_samples()` / `stream()`: Always send a revision strategy, defaulting to `LATEST_ONLY`. The service now rejects requests with an `UNSPECIFIED` revision strategy (`INVALID_ARGUMENT: stream_filter.revision_strategy must not be UNSPECIFIED`), which the client previously triggered whenever the parameter was left unset.
