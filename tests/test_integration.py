...
def make_location(
    ...
    market_area: MarketArea = MarketArea.EU_DE,
...
class TestUpdateMarketLocation:
    ...
    async def test_update_time_resolution_to_coarser_fails(
        self, client: MarketMeteringApiClient
    ) -> None:
        """Test that changing to a coarser resolution fails."""
        ml_ref = make_ref(enterprise_id=2, malo_id="DE_UPDATE_RES_FAIL_001")
        ml = make_location(
            display_name="Coarser Fails",
            time_resolution=TimeResolution.MIN_15,
        )
...
class TestUnimplementedRPCs:
    """Tests that unimplemented RPCs return proper errors."""

    async def test_list_market_locations_unimplemented(
        self, client: MarketMeteringApiClient
    ) -> None:
        """Test that list_market_locations returns UNIMPLEMENTED."""
        with pytest.raises(AioRpcError) as exc_info:
            await client.list_market_locations(enterprise_id=1)
        assert exc_info.value.code() == grpc.StatusCode.UNIMPLEMENTED

    async def test_activate_unimplemented(
        self, client: MarketMeteringApiClient
    ) -> None:
        """Test that activate_market_location returns UNIMPLEMENTED."""
        ml_ref = make_ref(enterprise_id=3, malo_id="DE_ACTIVATE_001")
        with pytest.raises(AioRpcError) as exc_info:
            await client.activate_market_location(market_location_ref=ml_ref)
        assert exc_info.value.code() == grpc.StatusCode.UNIMPLEMENTED

    async def test_deactivate_unimplemented(
        self, client: MarketMeteringApiClient
    ) -> None:
        """Test that deactivate_market_location returns UNIMPLEMENTED."""
        ml_ref = make_ref(enterprise_id=3, malo_id="DE_DEACTIVATE_001")
        with pytest.raises(AioRpcError) as exc_info:
            await client.deactivate_market_location(market_location_ref=ml_ref)
        assert exc_info.value.code() == grpc.StatusCode.UNIMPLEMENTED

    async def test_stream_samples_unimplemented(
        self, client: MarketMeteringApiClient
    ) -> None:
        """Test that stream_samples returns UNIMPLEMENTED."""
