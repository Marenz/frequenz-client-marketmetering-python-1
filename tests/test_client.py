...
class TestCreateMarketLocation:
    ...
    async def test_sends_correct_request(self) -> None:
        ...
        assert request.market_location_ref.market_location_id.value == "DE0000000001"
        assert request.market_location.display_name == "Test Location"
        assert request.market_location.market_area == pb.MARKET_AREA_EU_DE
...
class TestListMarketLocations:
    ...
    async def test_basic_list(self) -> None:
        ...
        detail_pb = pb.MarketLocationDetail(market_location=ml_pb)
        ...
        assert entries[0].market_location.market_area == MarketArea.EU_DE
...
class TestActivateMarketLocation:
    """Tests for activate_market_location."""

    async def test_sends_correct_request(self) -> None:
        """Test that activate sends the ref in market_location_refs."""
        client = _make_client()
        client.stub.ActivateMarketLocation = AsyncMock(
            return_value=pb.ActivateMarketLocationResponse()
        )
        ...
        assert request.market_location_refs[0].market_location_id.value == "DE_ACT_001"


class TestDeactivateMarketLocation:
    """Tests for deactivate_market_location."""

    async def test_sends_correct_request(self) -> None:
        """Test that deactivate sends the ref in market_location_refs."""
        client = _make_client()
        client.stub.DeactivateMarketLocation = AsyncMock(
            return_value=pb.DeactivateMarketLocationResponse()
        )
...
class TestUpsertSamples:
    ...
    async def test_yields_parsed_results(self) -> None:
        ...
        assert results[0].error_code == SampleUpsertErrorCode.UNSPECIFIED
